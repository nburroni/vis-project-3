...
var zoneColor = 0;
startTime = 10;
//var color_range = d3.scaleLinear().domain([0, 20000]).range(["rgba(253,212,158,.8)", "rgba(179,0,0,.8)"]);
...
var color_range = d3.scaleLinear().domain([0, 15000]).range(["rgba(253,187,132,.8)", "rgba(127,0,0,.8)"]);
var color_range2 = d3.scaleLinear().domain([0, 15000]).range(["rgba(250,159,181,.8)", "rgba(73,0,106,.8)"]);
var clicked = false;
var moused = false;
...
function initMap() {
    ...
    window.onDataReady = function(data, centers, GEO_JSON) {
        ...
        var max = Math.ceil(topCongestedList[0].Count_Num);
        //console.log(topCongestedList);
        //console.log(max);
        var num1 = Math.ceil((max / 6));
        var num2 = Math.ceil((max / 6) * 2);
        var num3 = Math.ceil((max / 6) * 3);
        var num4 = Math.ceil((max / 6) * 4);
        var num5 = Math.ceil((max / 6) * 5);
